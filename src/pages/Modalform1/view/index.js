...
import { Button, Row, Col, Form, Input, Select, Tooltip, Icon, message, Divider, Card, Drawer } from 'antd';
...
import FromDrawer from '../component/indexDrawer';
    ...
    return (
        <div style={{ width: '100%', margin: '50px auto' }}>
            <Card style={{ width: '45%', float: 'left', marginBottom: 20 }}>
                <Button onClick={() => setVisibleform1(true)}>点击弹框1</Button>
                <br />
                <br />
                <br />
                <p>Modal的button触发表单的提交测试</p>
            </Card>

            <Card style={{ width: '45%', float: 'right', marginBottom: 20 }}>
                <Button onClick={() => setVisibleform2(true)}>点击弹框2</Button>
                <br />
                <br />
                <br />
                <p>Modal的button设置为null,然后用表单的button进行提交,自定义样式.</p>
            </Card>

            <Card style={{ width: '45%', float: 'left' }}>
                <Button onClick={() => setVisibleform3(true)}>点击弹框3</Button>
                {/* <Button onClick={testFn}>Test</Button> */}
                <br />
                <br />
                <br />
                <p style={{ fontSize: 12 }}>
                    {/* Modal的button点击拿到表单实例对象, */}
                    ref获取表单对象,然后用`getFieldsValue`拿到所有值,进行提交.
                </p>
            </Card>

            <Card style={{ width: '45%', float: 'right' }}>
                <Button onClick={() => setVisibleform4(true)}>点击弹框4</Button>
                {/* <Button onClick={testFn}>Test</Button> */}
                <br />
                <br />
                <br />
                <p>设置Modal的button为数组对象,button的form属性与表单ID进行关联提交.</p>
            </Card>

            <Card style={{ width: '45%', float: 'left', marginTop: 20 }}>
                <Button onClick={showDrawer}>抽屉侧滑测试</Button>
            ...
            <Drawer
                ...
                footer={[
                    <Button key="cancel" type="default" style={{ marginRight: 20 }} onClick={onClose}>
                        取消
                    </Button>,
                    <Button type="primary" form="myDrawerForm" htmlType="submit" key="sure" onClick={onHandleSubmit}>
                        确定
                    </Button>,
                ]}>
                {open && <FromDrawer {...drawerProps5}></FromDrawer>}
            </Drawer>
        </div>
    );
};
